...
public class singlehasher
{
	double uniquewordcount = 0;
	double referncechangecount=0; // we will keep incrementing this everytime we change a reference or a pointer to keep track of data
	double comparecount=0; // we will keep incrementing this everything we preform a test for equivlance of strings
	node selector; // node for incrementing the list
	node[] hashtable; // table for storing data	
	public singlehasher() // make the table for the words to be hashed and stored in
	{
	...
	//table if there is one on there. If the hashcode we are looking at does not contain mutiple hashnodes then we will simply make a
	//new node that is equal to the hashcode index.
	{
		...
		else // if the hashcode isnt empty then we need to find the end of the node link in that hashcode
		{
			...
			referncechangecount++;
			node node = new node(input, selector); // when we are finally at the end make a new node with the string in it
			uniquewordcount++;
			referncechangecount++;
	...
	public int gethashcode(String tobehashed) // here we will get the hashcode for the table to store the word
	{
		int hashcode = 0; // make a new hashcode number to store the finished code
			...
			hashcode += (int)(tobehashed.charAt(0)); // cast the next letter as an int and add it to the hascode
